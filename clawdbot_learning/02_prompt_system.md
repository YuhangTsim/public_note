...
```
┌─────────────────────────────────────────────────────────┐
│  SYSTEM PROMPT (Generated by buildAgentSystemPrompt)    │
│                                                          │
...
│  9. Time: Timezone + current time                       │
│ 10. Reply Tags: [[reply_to_current]] syntax             │
│ 11. Messaging: Channel routing rules                    │
│ 12. Voice: TTS hints                                    │
│ 13. Reactions: Emoji guidance                           │
│ 14. Silent Replies: HEARTBEAT_OK token                  │
│ 15. Runtime: OS, arch, node, model info                 │
│                                                          │
│  ┌─────────────────────────────────────────────────┐   │
│  │  PROJECT CONTEXT (Injected Files)               │   │
│  │                                                  │   │
│  │  ## SOUL.md                                     │   │
│  │  [User's personality/identity instructions]     │   │
│  │                                                  │   │
│  │  ## AGENTS.md                                   │   │
...
<available_skills>
- **mcporter** (~/clawd/skills/mcporter) — Tool server runtime/CLI for managing external skill backends
- **peekaboo** (~/clawd/skills/peekaboo) — Fast macOS screenshots with optional AI vision analysis
- **camsnap** (~/clawd/skills/camsnap) — Capture frames, clips, or motion alerts from RTSP/ONVIF security cams
- **oracle** (~/clawd/skills/oracle) — OpenAI-ready agent CLI with session replay and browser control
- **eightctl** (~/clawd/skills/eightctl) — Control your sleep, from the terminal
- **imsg** (~/clawd/skills/imsg) — Send, read, stream iMessage & SMS
- **wacli** (~/clawd/skills/wacli) — WhatsApp CLI: sync, search, send
- **discord** (~/clawd/skills/discord) — Discord actions: react, stickers, polls
- **gog** (~/clawd/skills/gog) — Google Suite CLI: Gmail, Calendar, Drive, Contacts
- **spotify-player** (~/clawd/skills/spotify-player) — Terminal Spotify client to search/queue/control playback
- **sag** (~/clawd/skills/sag) — ElevenLabs speech with mac-style say UX
- **sonos-cli** (~/clawd/skills/sonos-cli) — Control Sonos speakers from scripts
- **blucli** (~/clawd/skills/blucli) — Play, group, and automate BluOS players from scripts
- **openhue-cli** (~/clawd/skills/openhue-cli) — Philips Hue lighting control for scenes and automations
- **openai-whisper** (~/clawd/skills/openai-whisper) — Local speech-to-text for quick dictation
- **gemini-cli** (~/clawd/skills/gemini-cli) — Google Gemini models from the terminal
- **bird** (~/clawd/skills/bird) — X/Twitter CLI to tweet, reply, read threads, and search
- **agent-tools** (~/clawd/skills/agent-tools) — Utility toolkit for automations and helper scripts
</available_skills>
...
| **Multi-channel** | ✅ 13+ channels | ❌ No | ❌ No | ⚠️ Varies |
| **Skills/Tools** | ✅ Dynamic, file-based | ⚠️ Limited | ⚠️ Limited | ⚠️ Varies |
| **Privacy** | ✅ Local-first | ❌ Cloud | ❌ Cloud | ⚠️ Varies |
| **Customization** | ✅ Markdown files | ❌ UI only | ❌ UI only | ⚠️ Code changes |
| **Mode switching** | ✅ Full/Minimal/None | ❌ No | ❌ No | ⚠️ Varies |
| **Context files** | ✅ Auto-injected | ❌ Manual | ❌ Manual | ⚠️ Varies |
